...
class CreateDonhangsanphamTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('donhangsanpham', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            $table->bigInteger('dh_ma')->autoIncrement()->comment('Mã đơn hàng, 1-Không xuất hóa đơn');
            $table->bigInteger('kh_ma')->comment('Khách hàng # kh_ma # kh_hoTen # Mã khách hàng');
            $table->dateTime('dh_thoiGianDatHang')->default(DB::raw('CURRENT_TIMESTAMP'))->comment('Thời điểm đặt hàng # Thời điểm đặt hàng');
            $table->dateTime('dh_thoiGianGiaoHang')->comment('Thời điểm giao hàng # Thời điểm giao hàng theo yêu cầu của khách hàng');
        ...
        DB::statement("ALTER TABLE `donhangsanpham` comment 'Đơn hàng # Đơn hàng'");
    ...
    {
        Schema::dropIfExists('donhangsanpham');
    }
}
